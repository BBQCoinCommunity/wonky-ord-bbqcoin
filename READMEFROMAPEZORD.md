...
`ord` relies on BBQCoin Core for private key management and transaction signing.
This has a number of implications that you must understand in order to use
`ord` wallet commands safely:

- BBQCoin Core is not aware of inscriptions and does not perform sat
  control. Using `bbqcoin-cli` commands and RPC calls with `ord` wallets may
...
- Because `ord` has access to your BBQCoin Core wallets, `ord` should not be
...
`ord` requires a synced `bbqcoind` node with `-txindex` to build the index of
satoshi locations. `ord` communicates with `bbqcoind` via RPC.

If `bbqcoind` is run locally by the same user, without additional
configuration, `ord` should find it automatically by reading the `.cookie` file
from `bbqcoind`'s datadir, and connecting using the default RPC port.

If `bbqcoind` is not on mainnet, is not run by the same user, has a non-default
