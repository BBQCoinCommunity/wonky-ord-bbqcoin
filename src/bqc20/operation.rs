...
use crate::bqc20::deploy::Deploy;
use crate::bqc20::errors::{JSONError};
use crate::bqc20::mint::Mint;
use crate::bqc20::OperationType;
use crate::bqc20::params::PROTOCOL_LITERAL;
...
pub(crate) fn deserialize_bqc20_operation(
    ...
    let raw_operation = match deserialize_bqc20(content_body) {
        Ok(op) => op,
        Err(e) => {
            return Err(e.into());
        }
    ...
    match action {
        ...
        Action::Transfer => match raw_operation {
            ...
            _ => Err(JSONError::NotBQC20Json.into()),
        },
    }
}

fn deserialize_bqc20(s: &str) -> Result<RawOperation, JSONError> {
    let value: Value = serde_json::from_str(s).map_err(|_| JSONError::InvalidJson)?;
    if value.get("p") != Some(&json!(PROTOCOL_LITERAL)) {
        return Err(JSONError::NotBQC20Json);
    }
