...
use crate::bqc20::operation::{Action, deserialize_bqc20_operation, InscriptionOp, Operation};
use crate::bqc20::transfer::Transfer;
use crate::bqc20::TransferInfo;
...
impl Message {
  pub(crate) fn resolve<'a, 'db, 'tx>(
    ...
    let bqc20_operation = match op.action {
      Action::New { inscription: _ } if sat_in_outputs => {
        match deserialize_bqc20_operation(
          new_inscriptions
          ...
          Ok(bqc20_operation) => bqc20_operation,
          _ => return Ok(None),
        }
      }
      // Transfered inscription operation.
      // Attempt to retrieve the `InscribeTransfer` Inscription information from the data store of BQC20.
      Action::Transfer => {
        match get_inscribe_transfer_inscription(bqc20_inscribe_transfer, op.inscription_id) {
